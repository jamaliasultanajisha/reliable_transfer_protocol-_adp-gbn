...
void send_window(void) {
    while (A.nextseq < A.buffer_next && A.nextseq < A.base + A.window_size) {
        ...
        fprintf(fp,"	send_window: send packet: seq= %d: %s\n", (packet->seqnum%8), packet->payload);
        tolayer3(0, *packet);
        if (A.base == A.nextseq) {
...
void A_output(struct msg message)
{
    if (A.buffer_next-A.base >= BUFSIZE) {
        fprintf(fp,"	A_output: buffer full. drop the message: %s\n", message.data);
        return;
    }
    else {
    	fprintf(fp,"	A_output: bufferred packet: seq= %d: %s\n", (A.buffer_next%8), message.data);
...
{
    fprintf(fp,"	B_output: uni-directional. ignore.\n");
}

/* called from layer 3, when a packet arrives for layer 4 */
void A_input(struct pkt packet)
{
    if (packet.checksum != create_checksum(&packet)) {
        fprintf(fp,"	A_input: packet corrupted. drop.\n");
        return;
    }
    if (packet.acknum < A.base) {
        fprintf(fp,"	A_input: got NACK. (ack%d). drop.\n", (packet.acknum%8));
       // printf("	A_input: got NACK. drop.\n");
        return;
    }
    fprintf(fp,"	A_input: got ACK. ack%d\n", (packet.acknum%8));
    A.base = packet.acknum + 1;
    if (A.base == A.nextseq) {
        stoptimer(0);
        fprintf(fp,"	A_input: stop timer\n");
        send_window();
    } 
    else {
        starttimer(0, A.estimated_rtt);
        fprintf(fp,"	A_input: timer + %f\n", A.estimated_rtt);
    }
}
...
{
    starttimer(0, A.estimated_rtt);
    fprintf(fp,"	A_timerinterrupt: timer + %f\n", A.estimated_rtt);
    for (int i = A.base; i < A.nextseq; i++) {
        struct pkt *packet = &A.packet_buffer[i % BUFSIZE];
        fprintf(fp,"	A_timerinterrupt: resend packet: seq=%d: %s\n", (packet->seqnum%8), packet->payload);
        tolayer3(0, *packet);
    }
}
...
{
    if (packet.checksum != create_checksum(&packet)) {
        fprintf(fp,"	B_input: packet corrupted. send NAK: (ack%d)\n", (B.packet_to_send.acknum%8));
        //printf("	B_input: packet corrupted. send NAK \n");
        tolayer3(1, B.packet_to_send);
        return;
    }
    if (packet.seqnum != B.expect_seq) {
        fprintf(fp,"	B_input: not the expected seq. send NAK: (ack=%d)\n", (B.packet_to_send.acknum%8));
       // printf("	B_input: not the expected seq. send NACK\n");
        tolayer3(1, B.packet_to_send);
        return;
    }

    fprintf(fp,"	B_input: recv packet: seq= %d: %s\n", (packet.seqnum%8), packet.payload);
    tolayer5(1, packet.payload);

    fprintf(fp,"	B_input: send ACK: ack%d\n", (B.expect_seq%8));
    B.packet_to_send.acknum = B.expect_seq;
    B.packet_to_send.checksum = create_checksum(&B.packet_to_send);
    tolayer3(1, B.packet_to_send);
...
void init() /* initialize the simulator */
{
    ...
    fprintf(fp,"-----  Stop and Wait Network Simulator Version 1.1 -------- \n\n");
    printf("Enter the number of messages to simulate: ");
    scanf("%d",&nsimmax);
    fprintf(fp, "The number of messages to simulate: %d\n",nsimmax);
    printf("Enter  packet loss probability [enter 0.0 for no loss]:");
    scanf("%f",&lossprob);
    fprintf(fp, "Packet loss probability [enter 0.0 for no loss]: %f\n",lossprob);
    printf("Enter packet corruption probability [0.0 for no corruption]:");
    scanf("%f",&corruptprob);
    fprintf(fp, "Packet corruption probability [enter 0.0 for no corruption]: %f\n",corruptprob);
    printf("Enter average time between messages from sender's layer5 [ > 0.0]:");
    scanf("%f",&lambda);
    fprintf(fp,"Average time between messages from sender's layer5 [ > 0.0]: %f\n",lambda);
    printf("Enter TRACE:");
    scanf("%d",&TRACE);
    fprintf(fp,"Trace: %d\n",TRACE);
