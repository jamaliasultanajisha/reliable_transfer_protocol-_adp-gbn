...
/*simplex transfer from a-to-B, there is no B_output()*/
...
void A_output(struct msg message)
{
    ...
    if (A.state != WAIT_LAYER5) {
        fprintf(fp,"	A_output : not yet acked. drop the message: %s\n", message.data);
        return;
    }
    fprintf(fp,"	A_output: send packet: %s\n", message.data);
    packet.seqnum = A.seq;
...
{
    fprintf(fp,"	B_output: uni-directional (from A to B). ignore.\n");
}
...
{
    if (A.state != WAIT_ACK) {
        fprintf(fp,"	A_input: uni-directional: A->B only. drop.\n");
        return;
    }
    ...
    if (packet.acknum != A.seq) {
        fprintf(fp,"	A_input: not the expected ACK. drop.\n");
        return;
    }
    //fprintf(fp,"	A_input: acked.\n");
    fprintf(fp,"	A_input: acked. receive ack%d.\n",A.seq);
    stoptimer(0);
    A.seq = 1 - A.seq;
    A.state = WAIT_LAYER5;
}
...
{
    if (A.state != WAIT_ACK) {
        fprintf(fp,"	A_timerinterrupt: not waiting for ACK anymore. ignore event.\n");
        return;
    }
    fprintf(fp,"	A_timerinterrupt: resend last packet: %s.\n", A.last_packet.payload);
...
{
    if (packet.checksum != create_checksum(&packet)) {
        fprintf(fp,"	B_input: packet corrupted. send NACK.\n");
        send_ack(1, 1-B.seq);
        return;
    }
    if (packet.seqnum != B.seq) {
        fprintf(fp,"	B_input: not the expected seq no. send NACK.\n");
        send_ack(1, 1-B.seq);
        return;
    }
    fprintf(fp,"	B_input: recv packet. message: %s\n", packet.payload);
...
int main()
{
    ...
    //FILE *fp;
    fp = fopen("output_adp.docx", "w");
   // ffprintf(fp, "This is being written in the file.");
